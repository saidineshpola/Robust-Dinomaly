...
def auroc_aupr_f1max_aupro_cuda(pr_list_px, gt_list_px):
    pr_list_px = pr_list_px.cpu().numpy()
    gt_list_px = gt_list_px.cpu().numpy()

    auroc_px = roc_auc_score(gt_list_px, pr_list_px)
    precision, recall, _ = precision_recall_curve(gt_list_px, pr_list_px)
    ap_px = auc(recall, precision)
    f1_scores = calculate_f1(precision, recall)
    f1_px = np.max(f1_scores)
    aupro_px = auc(recall, precision)

    return auroc_px, ap_px, f1_px, aupro_px


def auroc_aupr_f1max_cuda(pr_list_sp, gt_list_sp):
    pr_list_sp = pr_list_sp.cpu().numpy()
    gt_list_sp = gt_list_sp.cpu().numpy()

    auroc_sp = roc_auc_score(gt_list_sp, pr_list_sp)
    precision, recall, _ = precision_recall_curve(gt_list_sp, pr_list_sp)
    ap_sp = auc(recall, precision)
    f1_scores = calculate_f1(precision, recall)
    f1_sp = np.max(f1_scores)
...
def global_cosine_hm_percent(a, b, p=0.9, factor=0.):
    # A shape 2
    # b shape 2
    # A shape torch.Size([16, 768, 16, 16])
    # b shape torch.Size([16, 768, 16, 16])
    # import sys
    # sys.exit()
    cos_loss = torch.nn.CosineSimilarity()
    loss = 0
    for item in range(len(a)):
        ...
        # mean_dist = point_dist.mean()
        # std_dist = point_dist.reshape(-1).std()
        thresh = torch.topk(point_dist.reshape(-1),
                            k=int(point_dist.numel() * (1 - p)))[0][-1]
...
def cal_anomaly_map_v2(fs_list, ft_list, out_size=224, amap_mode='add'):
    ...
    for i in range(len(ft_list)):
        ...
        a_map = 1 - F.cosine_similarity(fs, ft)
        a_map = torch.unsqueeze(a_map, dim=1)
        a_map = F.interpolate(a_map, size=out_size // 4,
                              mode='bilinear', align_corners=False)
        a_map_list.append(a_map)

    anomaly_map = torch.stack(a_map_list, dim=-1).sum(-1)
    anomaly_map = F.interpolate(
        anomaly_map, size=out_size, mode='bilinear', align_corners=False)
        ...
        for img, gt, label, _ in dataloader:
            ...
            if feature_used == 'trained':
                anomaly_map, _ = cal_anomaly_map(
                    en[3:], de[3:], img.shape[-1], amap_mode='a', norm_factor=norm_factor)
            elif feature_used == 'freezed':
                anomaly_map, _ = cal_anomaly_map(
                    en[:3], de[:3], img.shape[-1], amap_mode='a', norm_factor=norm_factor)
            else:
                anomaly_map, _ = cal_anomaly_map(
                    en, de, img.shape[-1], amap_mode='a', norm_factor=norm_factor)
            ...
            if max_ratio <= 0:
                ...
                sp_score = np.sort(
                    anomaly_map)[-int(anomaly_map.shape[0] * max_ratio):]
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            ...
            if resize_mask is not None:
                ...
                # gt = transforms.Resize((resize_mask, resize_mask))
            ...
            if max_ratio == 0:
                ...
                sp_score = torch.sort(anomaly_map, dim=1, descending=True)[
                    0][:, :int(anomaly_map.shape[1] * max_ratio)]
...
def evaluation_batch_fast(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    with torch.no_grad():
        for img, gt, label, cls in dataloader:
            ...
            if max_ratio == 0:
                sp_score = torch.max(anomaly_map.flatten(1), dim=1)[
                    0].cpu().numpy()
            else:
                anomaly_map = anomaly_map.flatten(1)
                sp_score = torch.sort(anomaly_map, dim=1, descending=True)[
        ...
        auroc_px, ap_px, f1_px, aupro_px = auroc_aupr_f1max_aupro_cuda(
...
def evaluation_batch_loco(model, dataloader, device, _class_=None, max_ratio=0):
    ...
    with torch.no_grad():
        ...
        auroc_logic = roc_auc_score(
            np.array(gt_list_sp)[np.logical_or(
        ...
        auroc_struct = roc_auc_score(
            np.array(gt_list_sp)[np.logical_or(
                defect_type_list == 'good', defect_type_list == 'structural_anomalies')],
...
def evaluation_uniad(model, dataloader, device, _class_=None, reg_calib=False, max_ratio=0):
    ...
    with torch.no_grad():
        for img, gt, label, _ in dataloader:
            ...
            anomaly_map = torch.mean(F.mse_loss(
                de, en, reduction='none'), dim=1, keepdim=True)
            ...
            if reg_calib:
                if reg.shape[1] == 2:
                    ...
                    anomaly_map = (anomaly_map - reg_mean) / \
                        (reg_max - reg_mean)
                    # anomaly_map = anomaly_map - reg_max

                else:
                    reg = F.interpolate(
                        reg, size=img.shape[-1], mode='bilinear', align_corners=True)
    ...
    with torch.no_grad():
        for img, gt, label, img_path in dataloader:
            ...
            for i in range(0, anomaly_map.shape[0], 8):
                ...
                name = img_path[i].split(
                    '/')[-2] + '_' + img_path[i].split('/')[-1].replace('.png', '')
                cv2.imwrite(save_dir_class + '/' + name + '_img.png', im)
                cv2.imwrite(save_dir_class + '/' +
                            name + '_cam.png', hm_on_img)
        ...
        for img, gt, label, img_path in dataloader:
            ...
            for i in range(3):
                ...
                gt_resize = F.interpolate(
                    gt, size=en_feat.shape[2], mode='bilinear') > 0

                en_abnorm = en_feat.permute(
                    0, 2, 3, 1)[gt_resize.permute(0, 2, 3, 1)[:, :, :, 0]]
                en_normal = en_feat.permute(
                    0, 2, 3, 1)[gt_resize.permute(0, 2, 3, 1)[:, :, :, 0] == 0]

                de_abnorm = de_feat.permute(
                    0, 2, 3, 1)[gt_resize.permute(0, 2, 3, 1)[:, :, :, 0]]
                de_normal = de_feat.permute(
                    0, 2, 3, 1)[gt_resize.permute(0, 2, 3, 1)[:, :, :, 0] == 0]

                en_abnorm_list.append(F.normalize(
                    en_abnorm, dim=1).cpu().numpy())
                en_normal_list.append(F.normalize(
                    en_normal, dim=1).cpu().numpy())
                de_abnorm_list.append(F.normalize(
                    de_abnorm, dim=1).cpu().numpy())
                de_normal_list.append(F.normalize(
                    de_normal, dim=1).cpu().numpy())

            save_dir_class = os.path.join(save_dir, str(_class_))
            if not os.path.exists(save_dir_class):
                os.mkdir(save_dir_class)
            name = img_path[0].split(
                '/')[-2] + '_' + img_path[0].split('/')[-1].replace('.png', '')
...
def compute_pro(masks: ndarray, amaps: ndarray, num_th: int = 200) -> None:
    ...
    for th in np.arange(min_th, max_th, delta):
        ...
        df = pd.concat([df, pd.DataFrame([{"pro": mean(pros), "fpr": fpr,
