...
class JointTransform:
    """Apply the same spatial transformation to both image and mask."""
...
# def get_data_transforms_aug(size=256, isize=256, mean_train=None, std_train=None):
#     mean_train = [0.485, 0.456, 0.406] if mean_train is None else mean_train
#     std_train = [0.229, 0.224, 0.225] if std_train is None else std_train

#     spatial_transforms = [
#         JointTransform(transforms.RandomHorizontalFlip(), prob=0.5),
#         JointTransform(transforms.RandomVerticalFlip(), prob=0.5),
#         JointTransform(transforms.RandomRotation(20), prob=0.3)
#     ]
...
def get_data_transforms_train(size=256, isize=256, mean_train=None, std_train=None):
    mean_train = [0.485, 0.456, 0.406] if mean_train is None else mean_train
    std_train = [0.229, 0.224, 0.225] if std_train is None else std_train
...
#                  image_size=256, crop_size=224, mean_train=None, std_train=None):
#         self.img_path = os.path.join(
#             root, 'train') if phase == 'train' else os.path.join(root, 'test')
#         self.gt_path = os.path.join(
#             root, 'ground_truth') if phase != 'train' else None

#         self.image_size = image_size
#         self.crop_size = crop_size
#         self.mean_train = [0.485, 0.456,
#                            0.406] if mean_train is None else mean_train
#         self.std_train = [0.229, 0.224,
#                           0.225] if std_train is None else std_train
...
#             gt_tot_paths), "Mismatch between image and ground truth counts!"
...
#         if label == 0:
#             gt = torch.zeros((1, img.size[1], img.size[0]))
#         else:
#             gt = Image.open(gt_path).convert('L')

#         # Apply joint spatial transformations
...
class RealIADDataset(torch.utils.data.Dataset):
    def __init__(self, root, category, transform, gt_transform, phase):
        ...
        json_path = os.path.join(
            root, 'realiad_jsons', 'realiad_jsons', category + '.json')
        ...
        for sample in data_set:
            self.img_paths.append(os.path.join(
                root, 'realiad_1024', category, sample['image_path']))
            label = sample['anomaly_class'] != 'OK'
            if label:
                self.gt_paths.append(os.path.join(
                    root, 'realiad_1024', category, sample['mask_path']))
            else:
        ...
        for defect_type in defect_types:
            if defect_type == 'good':
                ...
                img_paths = glob.glob(os.path.join(
                    self.img_path, defect_type) + "/*.png")
                gt_paths = glob.glob(os.path.join(
                    self.gt_path, defect_type) + "/*/000.png")
                img_paths.sort()
                gt_paths.sort()
                img_tot_paths.extend(img_paths)
                gt_tot_paths.extend(gt_paths)
            ...
            if defect_type == 'good':
                ...
                for domain_type in domain_types:
                    img_paths = glob.glob(os.path.join(
                        self.img_path, defect_type, domain_type) + "/*.png")
                    gt_paths = glob.glob(os.path.join(
                        self.gt_path, defect_type, domain_type) + "/*.png")
                    img_paths.sort()
                    gt_paths.sort()
                    img_tot_paths.extend(img_paths)
                    gt_tot_paths.extend(gt_paths)
...
class MVTecDRAEMDataset(torch.utils.data.Dataset):
    ...
    def augment_image(self, image, anomaly_source_path):
        ...
        if no_anomaly > self.anomaly_ratio:
            ...
            perlin_scale = 6
            min_perlin_scale = 0
            anomaly_source_img = Image.open(anomaly_source_path).convert(
                'RGB').resize((self.size, self.size))
    ...
    def __getitem__(self, idx):
        ...
        anomaly_source_idx = torch.randint(
            0, len(self.anomaly_source_paths), (1,)).item()
        a_img, label = self.augment_image(
            img, self.anomaly_source_paths[anomaly_source_idx])

        img = self.transform(img)
        a_img = self.strong_transform(a_img)

        assert img.size()[1:] == a_img.size()[
            1:], "image.size != a_img.size !!!"
