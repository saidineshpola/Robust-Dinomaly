...
class ADerLoader(nn.Module):
    ...
    def init_model_invad(self, model_type='default'):
        ...
        in_strides = [2 ** (len(in_chas) - i - 1)
                      for i in range(len(in_chas))]  # [4, 2, 1]
        latent_channel_size = 16
        latent_spatial_size = self.size // (2 ** (1 + len(in_chas)))

        model_encoder = SimpleNamespace()
        model_encoder.name = name
        model_encoder.kwargs = dict(pretrained=True,
                                    checkpoint_path='',
                                    strict=False,
    ...
    def load_state_dict(self, model_path='ADer/model/pretrain/net_15.pth'):
